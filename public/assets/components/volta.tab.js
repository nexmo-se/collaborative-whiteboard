...
Volta.tab = (function () {
  ...
  Tabs.prototype = {
    ...
    toggle: function (linkElement) {
      ...
      var panelActiveClass = this.isJs
        ? _class.panelJsActive
        : _class.panelActive;

      if (
        !Volta._hasClass(linkElement, _class.linkDisabled) &&
        (!this._activeLink || this._activeLink !== linkElement)
      ) {
        if (this._activeLink) {
    ...
    _findNextTab: function (direction) {
      ...
      for (
        ...
        !nextTab && this._calculateIndex(i) !== currentIndex;
        i += direction
      ) {
